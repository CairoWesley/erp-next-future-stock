...
// Auto-importa dados do cadastro (Patient/Prescriber) na linha fp_patients.
frappe.ui.form.on('Sales Order Patient', {
    patient(frm, cdt, cdn) {
    ...
    prescriber(frm, cdt, cdn) {
        ...
        frappe.db.get_value('Prescriber', row.prescriber, ['full_name', 'council_type', 'council_number', 'council_state']).then((r) => {
            const d = (r && r.message) || {};
            frappe.model.set_value(cdt, cdn, 'prescriber_full_name', d.full_name || '');
